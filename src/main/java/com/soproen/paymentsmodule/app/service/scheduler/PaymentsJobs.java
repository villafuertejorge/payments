...
import com.soproen.paymentsmodule.app.service.paymentfile.HandlerConciliationPaymentFileService;
...
public class PaymentsJobs {
	...
	@Autowired
	private HandlerConciliationPaymentFileService handlerConciliationPaymentFileService;
	...
	@Scheduled(fixedRateString = "${app.calculate-payment-amount-job-fixed-rate}", initialDelayString = "${app.calculate-payment-amount-job-initial-delay}")
	public void calculatePaymentAmountJob() {
		try {
			log.info("launched calculatePaymentAmountJob ");
			handlerGeneratePaymentFileService.handlerCalculatePaymentAmount();
			log.info("done calculatePaymentAmountJob ");
		} catch (ServiceException e) {
	...
	public void conciliationPaymentsFileJob() {
		try {
			...
			handlerConciliationPaymentFileService.conciliateFile();
