...
public class CatalogController extends AbstractParentController {
	...
	public ResponseEntity<?> retrieveAllPrograms() {
		try {
			List<PayProgramDTO> programList = utilities.mapObject(catalogService.retrieveAllPrograms(),
					new TypeReference<List<PayProgramDTO>>() {
					});
	...
	public ResponseEntity<?> retrieveAllDistricts() {
		try {
			...
			List<PayDistrictDTO> districtList = utilities.mapObject(catalogService.retrieveAllDistricts(),
					new TypeReference<List<PayDistrictDTO>>() {
					});
	...
	public ResponseEntity<?> retrieveComplianceTermsBygeographicalLocation(
		...
		try {
			...
			return new ResponseEntity<Map<String, Object>>(super.responseError("Failed - retrieve districts by id "),
					HttpStatus.OK);
		}
	}

	@GetMapping("/retrieveClosedComplianceTerms/{termType}/{idDistrict}")
	public ResponseEntity<?> retrieveClosedComplianceTerms(
			@PathVariable(name = "termType", required = true) String termType,
			@PathVariable(name = "idDistrict", required = true) String idDistrict) {
		try {

			String url = endPointComplianceModuleRetrieveClosedTerms.replace("{TERM_TYPE}", termType)
					.replace("{DISTRICT_ID}", idDistrict);
			ResponseEntity<CompTermDTO[]> response = restTemplateCompliance.getForEntity(url, CompTermDTO[].class);

			if (response.getStatusCode().equals(HttpStatus.OK)) {
				return new ResponseEntity<Map<String, Object>>(
						super.responseOK("OK", Arrays.asList(response.getBody())), HttpStatus.OK);
			} else {
				return new ResponseEntity<Map<String, Object>>(
						super.responseError("Failed - retrieve closed compliance terms"), HttpStatus.OK);
			}
	...
	@GetMapping("/retrieveAllActiveFormulaByProgram/{idProgram}")
	public ResponseEntity<?> retrieveAllActiveFormulaByProgram(
			@PathVariable(name = "idProgram", required = true) Long idProgram) {
		try {

			List<PayFormulaDTO> districtList = utilities.mapObject(
					catalogService.retrieveAllActiveFormulasByProgram(PayProgram.builder().id(idProgram).build()),
					new TypeReference<List<PayFormulaDTO>>() {
					});
			return new ResponseEntity<Map<String, Object>>(super.responseOK("OK", districtList), HttpStatus.OK);
		} catch (ServiceException e) {
			log.error("/retrieveAllActiveFormulaByProgram/idProgram = {}", e.getMessage());
			return new ResponseEntity<Map<String, Object>>(super.responseError("Failed - retrieve active formulas by program "),
					HttpStatus.OK);
		}
	}
